...
⇒ 일반적으로 생성하는 문자열 변수 안에는 메서드와 프로퍼티가 자동 내장 
...
이 기능들의 공통점은 원본 내용은 **절대 변하지않고, 가공 결과를 리턴한다.**
...
console.log("문자열 : %s", msg);

//  문자열의 글자 수를 가져온다.
//  --> 공백과 특수문자 포함
const len = msg.length;
console.log("문자열의 길이 %d", len);
...
//  두 번째 숫자 값은 첫 번재 파라미터의 글자를 찾기 시작하는 위치를 의미한다.
...
//  단 이 위치를 문장ㄹ의 끝에서 부터 세는 것이 아니라 문자열의 처음부터 센다.
//  찾지 못할 경우 -1을 반환.
