...
import image from '../assets/img/hamburger.jpg';
const MyEffect = () => {
  // 이미지의 밝기를 위한 상태값
  ...
  React.useEffect(() => {
    ...
    //  window 객체 등과 같이 DOM이외의 객체에게 이벤트를 적용할 경우 무조건 useEffect 사용
  ...
  /** 이 컴포넌트가 화면에 막 등장할 때와 state, props값이 변경될 때마다 매번 실행 */
  React.useEffect(() => {
    console.log('[MyEffect2] %s ::: 화면에 컴포넌트가 처음 로드되거나 state, props 중 하나라도 변경될 경우 호출됨', new Date());
  });

  /** 이 컴포넌트가 화면에 막 등장할 때와 특정 state, props값이 변경될 때만 다 매번 실행 */
  React.useEffect(() => {
    console.log('[MyEffect3] %s ::: myBrightness 변경', new Date());
  }, [myBrightness]);
  ...
  React.useEffect(() => {
    return () => {
      console.log('[MyEffect4] %s ::: 이 컴포넌트가화면에서 사라지기 직전에 처리되어야 할 기능', new Date());
    };
  });
  return (
    <div>
